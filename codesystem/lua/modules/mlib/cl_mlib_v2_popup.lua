function MLIB:PopUPV2(header,color, colorhoverd, text, content , confirm_text, denie_text, func_accept, func_deny) 
            ...
            if (content == "selection") then 
                self.select = vgui.Create("DComboBox", self)
                self.select:SetSize(self:GetWide() - 10,50)
                self.select:SetPos(5,70)
                self.select:SetValue(header)
...
concommand.Add("testpopup", function()
